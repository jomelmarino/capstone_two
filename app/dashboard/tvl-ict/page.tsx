...
export default function SubjectScreen() {
  ...
  const filteredStudents = groupedStudents['TVL-ICT']
    ? groupedStudents['TVL-ICT'].filter(student =>
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      {/* Header - Vibrant Gradient and Elevated */}
      <div className="bg-gradient-to-r from-teal-500 to-teal-700 pt-12 pb-8 px-6 shadow-xl rounded-b-3xl">
        <div className="flex items-start mb-6">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-teal-200 text-sm font-medium mt-1">
              Technical-Vocational-Livelihood Track, ICT Strand
            </p>
          </div>
        </div>

        {/* Search Bar - White, Shadowed, and Iconized (No Package) */}
        <div className="relative bg-white rounded-xl shadow-lg focus-within:ring-4 focus-within:ring-teal-300 transition duration-200">
      ...
      <div className="flex-1 px-4 py-6 overflow-y-auto">
        ...
        <div className="bg-white rounded-xl p-5 shadow-lg border border-gray-200">
          <h2 className="text-2xl font-bold text-teal-700 mb-4 border-b pb-2 border-gray-200">
          ...
          {loading ? (
          ...
          ) : groupedStudents['TVL-ICT'] && groupedStudents['TVL-ICT'].length > 0 ? (
            <div>
              <p className="text-md font-semibold text-gray-700 mb-4">
                Strand: <span className="text-teal-600 font-extrabold">TVL-ICT ({filteredStudents.length} Students)</span>
              </p>
              ...
              {filteredStudents.length > 0 ? (
                <div className="space-y-3">
                  {filteredStudents.map((student, index) => (
                    <div
                      key={student.lrn}
                      className="flex items-center p-3 bg-teal-50 hover:bg-teal-100 transition duration-150 rounded-lg border border-teal-200 shadow-sm"
                    >
                      <span className="text-teal-500 text-base font-bold mr-4 w-6 text-center">
                        {index + 1}.
                      </span>
                ...
                <p className="text-center py-4 text-gray-500 italic">
                  No students matched your search query in TVL-ICT.
                </p>
              )}
            </div>
          ) : (
            <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
              <p className="text-gray-600 font-medium">
                😔 No approved TVL-ICT students yet.
              </p>
            </div>
          )}
        </div>
